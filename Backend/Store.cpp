...
Store::Store() : storageInventory(nullptr) {
    // Initialize storage with 25 slots (matching StoreScene)
...
bool Store::addToStorage(Plant* plant) 
{
    if (!plant || !storageInventory) 
    {
        return false;
    }
    
    return storageInventory->add(plant);
}